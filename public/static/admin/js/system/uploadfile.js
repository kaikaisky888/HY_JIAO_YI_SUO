        ...
        index: function () {
            ea.table.render({
                ...
                cols: [[
                    ...
                    {field: 'url', minWidth: 80, search: false, title: '图片信息', imageHeight: 40, align: "center", templet: function(d) {
                        var url = d.url || '';
                        if (url && url.indexOf('http') !== 0) { url = '/' + url.replace(/^\//, ''); }
                        return '<img style="max-width:100px;max-height:40px;" src="' + url + '" data-image="图片">';
                    }},
                    {field: 'url', minWidth: 120, title: '保存地址', templet: function(d) {
                        var url = d.url || '';
                        var fullUrl = url;
                        if (url && url.indexOf('http') !== 0) { fullUrl = '/' + url.replace(/^\//, ''); }
                        return '<a class="layuimini-table-url" href="' + fullUrl + '" target="_blank">' + url + '</a>';
                    }},
