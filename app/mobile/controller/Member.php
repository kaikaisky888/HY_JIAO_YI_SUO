...
class Member extends MobileController
{
    ...
    public function account()
    {
        ...
        $this->assign(['phone'=>$phone,'email'=>$email,'paypwd'=>$paypwd]);
        if($safe >= 80){
    ...
    public function getemailcode()
    {
        if(request()->isPost()){
            ...
            if(!$email){
                return $this->error(lang('wicket_page.register_email'));
            }
            $checkmail = "/^([\.a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\.[a-zA-Z0-9_-])+/";
            if(!preg_match($checkmail,$email)){
                return $this->error(lang('wicket_page.register_email'));
            }

            $userst = \app\admin\model\MemberUser::where(['email'=>$email])
                ->whereOr(['username'=>$email])
                ->find();
            if($userst){
            ...
            $sms_send_count = sysconfig('base','send_count');
            $count=\app\admin\model\SystemCode::where(['phone'=>$email,'useable'=>0])->count('id');
            if($sms_send_count > 0 && $count > $sms_send_count){
                return $this->error(fox_all_replace(lang('wicket_page.code_send_num'),$count));
            }
            ...
            $cdata['ip'] = getRealIp();
            if(sysconfig('site','site_type')=='online'){
                if(FoxCommon::sendMail($email,$title,$content)){
                    \app\admin\model\SystemCode::create($cdata);
    ...
    public function setemail()
    {
        if(request()->isPost()){
            ...
            $codes = \app\admin\model\SystemCode::where(['phone'=>$email,'code'=>$code,'useable'=>0])
            ...
            if($save){
                return $this->success(lang('member.member_email_setok'),[],(string)url('member/account'));
            }
            return $this->error(lang('public.do_fail'));
        }
    }
    ...
    public function authset()
    {
        ...
        $cardRow = $this->card_model::where('uid',$this->member['id'])->find();
        $card = $cardRow ? $cardRow->toArray() : [
            'status' => 0,
            'card_a' => null,
            'card_b' => null,
            'card_c' => null,
        ...
        if(request()->isPost()){
            ...
            if($cardRow){
                if($cardRow['status']==1){
                    ...
                    try {
                        $save = $this->card_model::update($data,['id'=>$cardRow['id']]);
                    } catch (\Exception $e) {
                        return $this->error(lang('public.do_fail'));
                    }
                    if($save){
    ...
    public function upload_card()
    {
        ...
        if(!$file){
            return $this->up_message(1,'请选择图片文件');
        }
        
        // 获取会员ID
        $member_id = $this->member['id'] ?? 0;
        if(!$member_id){
            return $this->up_message(1,'会员信息错误');
        }
        ...
        try {
            ...
            $data['paths'] = 'upload/cards/'.$member_id;
        ...
        try {
            ...
            // 手动创建目标目录（递归创建，解决服务器权限问题）
            $realDir = app()->getRootPath() . 'public/' . $paths;
            if(!is_dir($realDir)){
                mkdir($realDir, 0755, true);
            }
            
            // 上传文件
            $savename = $disk->putFile($paths, $file, function() use ($fileName){
                return $fileName;
            });
            ...
            if($savename){
                ...
                $rdata['url'] = $path;
                // public 目录的文件可以直接访问
//                $rdata['src'] = request()->domain() . $path;
                $rdata['src'] =  $path;
                return $this->up_message(0,lang('public.upload_ok'),$rdata);
            }else{
                return $this->up_message(1,'文件上传失败，请重试');
            }
        } catch (\Exception $e) {
            return $this->up_message(1,'上传过程出错：'.$e->getMessage());
        }
    }
    ...
    public function upload_video()
    {
        $file = request()->file('file');
        if(!$file){
            return $this->up_message(1,'请选择视频文件');
        }
        // 检查文件大小（max 10MB）
        if($file->getSize() > 10 * 1024 * 1024){
            return $this->up_message(1,'视频文件不能超过10MB');
        }
        // 检查文件类型
        $ext = strtolower($file->getOriginalExtension());
        if($ext != 'mp4'){
            return $this->up_message(1,'仅支持MP4格式视频');
        }
        $paths = 'upload/videos/'.$this->member['id'];
        $fileName = date('YmdHis').rand(1000,9999);
        $savename = \think\facade\Filesystem::disk('public')->putFile($paths, $file, function() use ($fileName){
            return $fileName;
        });
        ...
        if($savename){
            ...
            $rdata['url'] = $path;
            $rdata['src'] = $path;
            return $this->up_message(0,'视频上传成功',$rdata);
        }else{
            return $this->up_message(1,'视频上传失败');
        }
    }
    ...
    {
        $cardRow = $this->card_model::where('uid',$this->member['id'])->find();
        if(!$cardRow || $cardRow['status'] != 1){
            return $this->error('请先完成基础身份认证');
        }
        $card = $cardRow->toArray();
        // 初始化高级认证字段默认值
        if(!isset($card['advanced_status'])) $card['advanced_status'] = 0;
        if(!isset($card['advanced_video'])) $card['advanced_video'] = '';
        if(!isset($card['advanced_remark'])) $card['advanced_remark'] = '';

        $this->assign(['card'=>$card]);

        if(request()->isPost()){
            $data = request()->post();
            if(empty($data['advanced_video'])){
                return $this->error('请上传认证视频');
            }
            $updateData = [
                'advanced_video' => $data['advanced_video'],
                'advanced_status' => 1, // 审核中
                'advanced_time' => time(),
                'advanced_do_time'  => 0,
                'advanced_remark' => '',
            ];
            try {
                $save = $this->card_model::update($updateData, ['id'=>$cardRow['id']]);
            } catch (\Exception $e) {
                return $this->error('提交失败');
            }
            if($save){
                return $this->success('高级认证已提交，预计7个工作日内完成审核',[],(string)url('member/authset'));
            }
            return $this->error('提交失败');
        }

        $web_name = '高级认证-'.$this->web_name;
        $this->assign(['web_name'=>$web_name,'leftmenu'=>'authset']);
        return $this->fetch('advancedauth');
    }
